/**
 * FlowField Class Implementation with Color Gradient
 *
...
class FlowField {
    ...
    setup() {
      ...
      colorMode(HSB, 360, 100, 100); // Set color mode to HSB for gradient effect
    ...
    update() {
      ...
      for (let i = 0; i < this.points.length; i++) {
        ...
        // Set fill color based on point's position to create a gradient effect
        let hue = map(this.points[i].x, 0, width, 0, 360); // Hue changes based on x-position
        let brightness = map(this.points[i].y, 0, height, 50, 100); // Brightness changes based on y-position
        fill(hue, 80, brightness); // Set fill color with hue, saturation, and brightness
